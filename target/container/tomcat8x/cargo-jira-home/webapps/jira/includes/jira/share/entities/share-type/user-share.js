define('jira/share/entities/share-type/user-share', ['jira/share/entities/display', 'jira/share/entities/share-permission', 'jira/share/i18n', 'jquery'], function (Display, SharePermission, i18n, jQuery) {
    ...
    UserShare.prototype = {
        ...
        initialise: function initialise() {
            ...
            if (this.userSelect) {
                jQuery(this.userSelect).on('selected', function (event, selectedItem, singleSelectController) {
                    that.value = selectedItem.properties ? selectedItem.properties.userKey : selectedItem.value();
        ...
        getDisplayFromUI: function getDisplayFromUI(preventClear) {
            var result = null;
            if (this.singleSelectController && this.value) {
                if (!preventClear) {
                    this.singleSelectController.clear();
                }
                var newPermission = new SharePermission(this.type, this.value, null);
                result = new Display(this.getDisplayString(this.value), this.getDescriptionString(this.value, true), this.singleton, newPermission);
            }
            return result;
        },
        /**
